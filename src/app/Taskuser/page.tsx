...
const ActivityPage = () => {
  ...
  const handleEdit = (activity: Activity) => {
    ...
    if (typeof activity.description === 'string') {
      ...
      splitDescriptions = Array.isArray(activity.description) ? activity.description : [activity.description || ""];
    }
              ...
              <div className="flex items-center justify-between w-full px-4 gap-4">
                <div className="relative inline-block">
                  <div className="flex justify-center w-full">
                    <button
                      ...
                      className="w-full max-w-xs flex items-center justify-center px-7 py-1 text-lg font-bold text-white bg-blue-600 border-2 border-blue-300 shadow-lg transition-all duration-300 ease-in-out cursor-pointer hover:bg-blue-500 hover:border-blue-200 hover:shadow-xl active:bg-blue-400 active:shadow-none active:translate-y-1"
                      style={{
                  ...
                  {open && (
                    <div
                      className={`absolute bg-blue-100 p-2 rounded-lg shadow-lg z-50 border border-blue-300 transition-transform duration-300 ease-in-out 
                        ${isMobile
                    ...
                    >
                      <div className={`grid ${isMobile ? "grid-cols-1" : "grid-cols-4"} gap-2`}>
                        {statuses.map((status) => (
                          <button
                            key={status}
                            className={`p-3 rounded-md bg-blue-100 text-blue-600 hover:bg-blue-200 transition flex items-center justify-center 
                ...
                <div className="flex justify-center w-full">
                  <button
                    ...
                    className="w-full max-w-xs flex items-center justify-center px-6 py-1 text-lg font-bold text-white bg-blue-600 border-2 border-blue-300 shadow-lg transition-all duration-300 ease-in-out cursor-pointer hover:bg-blue-500 hover:border-blue-200 hover:shadow-xl active:bg-blue-400 active:shadow-none active:translate-y-1"
                    style={{
  ...
  <div className="min-w-full max-w-6xl p-4 sm:p-6 rounded-lg shadow-lg border border-blue-300">
    {currentActivities.length > 0 && (
      <div className="relative p-6 sm:p-8 bg-white text-gray-900 rounded-lg shadow-lg flex flex-col w-full">
        <span
          className={`absolute top-4 right-4 px-3 py-1 text-sm sm:text-lg font-semibold rounded-full border border-blue-300 uppercase ${currentActivities[0].status === 'pending'
        ...
        <h1 className="text-2xl sm:text-4xl font-bold mt-6 mb-3 text-center">
          {currentActivities[0].title}
        </h1>

        <div className="mt-4 mb-6 text-center">
          <p className="text-blue-600 text-sm sm:text-lg font-semibold">
  ...
  <div className="overflow-x-auto rounded-lg border border-blue-300 shadow-lg bg-blue-50">
    <table className="w-full text-left border-collapse">
      <thead className="bg-blue-100 text-blue-700 uppercase text-sm">
        <tr>
          <th className="p-3 border-blue-300 w-12 text-center"></th>
          <th className="p-3 border-b border-blue-300 text-center">Description list</th>
        </tr>
      </thead>
      <tbody>
        {currentActivities[0].description
          ...
          .map((item, index) => (
            <tr key={index} className="hover:bg-blue-50 transition duration-300 ease-in-out">
              <td className="p-3 border-t border-r border-blue-200 text-center">{index + 1}</td>
              <td className="p-3 border-t border-blue-200">{item.trim()}</td>
            </tr>
          ))}
      </tbody>
    </table>
        ...
        <div className="flex flex-wrap justify-center gap-2 sm:gap-4 mt-4">
          {currentActivities[0].status !== 'overdue' && (
            <button
              ...
              className="px-4 py-2 text-sm rounded-full text-white bg-blue-600 transition hover:bg-blue-500"
            >
              ✏️ Edit
            </button>
          )}
          ...
          {currentActivities[0].status === 'pending' && !currentActivities[0].archive && (
            <button
              ...
              className="px-4 py-2 text-sm rounded-full text-white bg-green-600 transition hover:bg-green-500"
            >
              ✅ Done
            </button>
          )}
          ...
          {currentActivities[0].archive ? (
            <button
              ...
              className="px-4 py-2 text-sm rounded-full               text-white bg-yellow-600 transition hover:bg-yellow-500"
            >
              🔄 Restore
            </button>
          ) : (
          ...
          <button
            ...
            className="px-4 py-2 text-sm rounded-full text-white bg-red-600 transition hover:bg-red-500"
          >
            🗑️ Delete
          </button>
        </div>
        ...
        />
          ...
          <div className="flex justify-between items-center mt-4">
            <button
              ...
              className="bg-blue-600 p-2 rounded text-white text-xs font-bold flex items-center gap-1 transition-transform duration-300 hover:scale-105 active:scale-95 shadow-lg disabled:opacity-50"
            >
              <ChevronLeft size={16} /> Prev
            </button>
            <span className="self-center text-gray-900 text-xs font-bold">
